...
| Фаза 5, гейты (`05_vertical_saas_web/verification/GATES.yaml`) | 11 гейтов: `lint_ts`, `typecheck_ts`, `prisma_validate`, `prisma_generate`, `test_unit`, `test_contract`, `build_nextjs`, `docker_build`, `test_e2e` (Playwright), `security_audit` (`pnpm audit` + Semgrep), `bundle_analyze` | В `verticals/saas_web/verification/GATES.yaml` выброшены `test_contract`, `test_e2e`, Semgrep, `bundle_analyze` с формулировкой «инструментов нет в стеке»; `docker_build` перенесён в CI сгенерированного проекта; `prisma_generate` стал post_script скилла |
...
2. Фаза 5: `add_stripe`, `add_playwright_e2e`, `add_admin_dashboard`; промпт Reviewer; few-shot; вернуть гейты `test_contract`, `test_e2e`, Semgrep, `bundle_analyze`.
